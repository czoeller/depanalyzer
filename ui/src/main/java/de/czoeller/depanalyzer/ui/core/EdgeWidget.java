    ...
    private void updateAppearance () {
        ...
        if (state != DISABLED) {
            ...
            if (isConflict) {
                ...
                DependencyNode parent = grScene.getGraphNodeRepresentant(includedDepN).getArtifactParent(grScene.graph());
                String version = includedDepN.getArtifact().getVersion();
                String requester = parent != null ? parent.getArtifact().getArtifactId() : "???";
                String confText = edgeConflictType == ArtifactGraphNode.CONFLICT ? Bundle.TIP_VersionConflict(version, requester) : Bundle.TIP_VersionWarning(version, requester);
